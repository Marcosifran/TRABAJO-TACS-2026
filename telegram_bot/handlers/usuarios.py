...
async def cmd_faltantes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if status != 200:
        ...
        return
    if not data:
        await update.message.reply_text("No tenés faltantes registrados. Usá /agregar_faltante numero")
        return
...
async def cmd_agregar_faltante(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Uso: /agregar_faltante numero\nEjemplo: /agregar_faltante 42")
        return
    try:
    ...
    if status == 201:
        await update.message.reply_text(f"✅ Figurita #{numero} marcada como faltante.")
    else:
        await update.message.reply_text(f"❌ {fmt_error(data)}")
