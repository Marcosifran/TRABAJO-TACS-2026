...
async def cmd_buscar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Uso: /buscar numero\nEjemplo: /buscar 10")
        return
    try:
        numero = int(context.args[0])
    except ValueError:
        await update.message.reply_text("❌ El numero debe ser un entero.")
        return
    ...
    if status == 200:
        ...
        await update.message.reply_text(f"🔎 Figurita #{numero}\nJugador: {jugador}\nEquipo: {equipo}")
    elif status == 404:
        await update.message.reply_text(f"❌ La figurita #{numero} no existe en el maestro.")
    else:
...
async def cmd_equipos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if status != 200:
        ...
        return
    if not data:
        await update.message.reply_text("No hay equipos en el maestro.")
        return
    equipos = data if isinstance(data, list) else data.get("equipos", [])
    await update.message.reply_text("🌍 Equipos:\n" + "\n".join(f"  • {e}" for e in equipos))
